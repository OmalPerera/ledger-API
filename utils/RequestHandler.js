...
class RequestHandler {
	...
	validateJoi(err, status, errorType, errorMessage) {
		if (err) { console.log(`error in validating request : ${errorMessage}`, 'warn'); }
	...
	sendSuccess(res, message, status) {
		console.log(`a request has been made and proccessed successfully at: ${new Date()}`, 'info');
	...
	sendError(req, res, error) {
		console.log(`error ,Error during processing request: ${`${req.protocol}://${req.get('host')}${req.originalUrl}`} details message: ${error.message}`, 'error');
